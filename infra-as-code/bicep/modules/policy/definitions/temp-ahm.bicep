...
param policyName string = 'deploy-cloudhealth-healthmodel'

@description('Policy display name.')
param displayName string = 'Deploy Microsoft CloudHealth health model with discovery rules'

resource policyDefinition 'Microsoft.Authorization/policyDefinitions@2026-01-01-preview' = {
  name: policyName
  properties: {
    displayName: displayName
    description: 'Deploys a Microsoft.CloudHealth health model and discovery rule when missing.'
    policyType: 'Custom'
    mode: 'All'
    metadata: {
      category: 'Monitoring'
    ...
    parameters: {
      ...
      resourceGraphQuery: {
        type: 'String'
        metadata: {
          displayName: 'Resource Graph query'
          description: 'KQL query that must return at least an id column.'
        }
      }
    }
    policyRule: {
          ...
          deployment: {
            ...
            properties: {
              ...
              parameters: {
                ...
                discoveryScope: {
                  value: '[parameters(\'discoveryScope\')]'
                }
                discoveryRuleName: {
                ...
                authenticationSetting: {
                  value: '[parameters(\'authenticationSetting\')]'
                }
                addRecommendedSignals: {
              ...
              template: {
                ...
                parameters: {
                  healthModelName: { type: 'string' }
                  location: { type: 'string' }
                  userAssignedIdentityId: { type: 'string' }
                  discoveryScope: { type: 'string' }
                  discoveryRuleName: { type: 'string' }
                  discoveryRuleDisplayName: { type: 'string' }
                  authenticationSetting: { type: 'string' }
                  addRecommendedSignals: { type: 'string' }
                  discoverRelationships: { type: 'string' }
                  resourceGraphQuery: { type: 'string' }
                }
                resources: [
                  {
                    type: 'Microsoft.CloudHealth/healthmodels'
                    ...
                    properties: {
                      discovery: {
                        ...
                        scope: '[parameters(\'discoveryScope\')]'
                      }
                    }
                  }
                  {
                    ...
                    apiVersion: '2025-05-01-preview'
                    name: '[concat(parameters(\'healthModelName\'), \'/\', parameters(\'discoveryRuleName\'))]'
                    properties: {
                      addRecommendedSignals: '[parameters(\'addRecommendedSignals\')]'
                      authenticationSetting: '[parameters(\'authenticationSetting\')]'
